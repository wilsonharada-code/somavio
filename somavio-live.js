/* SomaVio — V1.1 EN/JA language layer + navigation/context interactions
   Updated: 2026-06-19
   Scope: My Trip label, weather/status copy support, V1.1 navigation alignment.
*/
(() => {
  'use strict';
  ...
  const TRANSLATIONS = {
  "en": {
    "nav_explore_canmore": "Explore Canmore",
    "nav_outdoor_adventures": "Outdoor Adventures",
    "nav_local_rules": "Local Rules",
    "nav_plans_changed": "Plans Changed?",
    "nav_plan_your_day": "My Trip",
    "nav_guesthub_signin": "Guest Hub / Sign In",
    "nav_guesthub": "Guest Hub",
    "drawer_nav_label": "Navigate",
    "drawer_language": "Language",
    "drawer_footer": "SomaVio · Local guidance for Alberta\n© 2026",
    "language_english": "English",
    "language_japanese": "日本語",
    "context_return_guest": "← Back to Guest Hub",
    "context_return_hotel": "← Back to Hotel Website",
  ...
  "ja": {
    "nav_explore_canmore": "キャンモアを探索",
    "nav_outdoor_adventures": "アウトドア体験",
    "nav_local_rules": "ローカルルール",
    "nav_plans_changed": "予定変更？",
    "nav_plan_your_day": "マイトリップ",
    "nav_guesthub_signin": "ゲストハブ / サインイン",
    "nav_guesthub": "ゲストハブ",
    "drawer_nav_label": "ナビゲーション",
    "drawer_language": "言語",
    "drawer_footer": "SomaVio · アルバータのローカルガイダンス\n© 2026",
    "language_english": "English",
    "language_japanese": "日本語",
    "context_return_guest": "← ゲストハブへ戻る",
    "context_return_hotel": "← ホテルサイトへ戻る",
  ...
  "plans-changed": {
    "en": {
      "plans_context": "Canmore · Adaptive planning",
      "plans_back": "← Back to Hotel Website",
      "plans_hero_label": "Adaptive Route Desk · Demo",
      "plans_hero_title": "Plans changed? That’s part of the adventure.",
      "plans_hero_sub": "Mountain days shift fast. Weather, wildlife alerts, road conditions or tired legs can change the original plan — so SomaVio helps visitors pivot with calm, curated alternatives for the moment they are actually in.",
      "plans_weather_label": "Current demo snapshot",
      "plans_weather_value": "☀️ Canmore · 18°C · Sunny Day",
      "plans_weather_note": "Live integrations can connect this layer to official weather, alerts and partner availability.",
      "plans_scenario_label": "Simulate Scenario (Demo Only)",
      "plans_filter_label": "Show:",
      "plans_results_all": "adaptive options ready now",
      "plans_section_kicker": "Flexible options",
      "plans_section_title": "Keep the day moving without forcing the original plan.",
      "plans_section_sub": "These cards demonstrate how SomaVio can hide exposed mountain plans during poor conditions and surface safer, warmer or lower-friction alternatives instantly.",
      "plans_public_tag": "Open to Public · Non-Guests Welcome",
      "plans_save": "+ Trip",
      "plans_details": "Details"
    },
    "ja": {
      "plans_context": "キャンモア · 柔軟なプラン変更",
      "plans_back": "← ホテルサイトへ戻る",
      "plans_hero_label": "アダプティブル・ルートデスク · デモ",
      "plans_hero_title": "予定変更？それも旅の一部です。",
      "plans_hero_sub": "山の一日はすぐに変わります。天候、野生動物アラート、道路状況、疲労で元の予定を変える必要がある時、SomaVioはその瞬間に合った落ち着いた代替案を提示します。",
      "plans_weather_label": "現在のデモ状況",
      "plans_weather_value": "☀️ キャンモア · 18°C · 晴れ",
      "plans_weather_note": "本番では公式天気、アラート、パートナー空き状況との連携を想定できます。",
      "plans_scenario_label": "シナリオをシミュレーション（デモのみ）",
      "plans_filter_label": "表示:",
      "plans_results_all": "今使える柔軟な選択肢",
      "plans_section_kicker": "柔軟な選択肢",
      "plans_section_title": "元の予定にこだわらず、一日を前に進める。",
      "plans_section_sub": "このカード群は、悪天候時に露出の高い山岳プランを隠し、より安全で温かく、負担の少ない代替案を即座に表示するデモです。",
      "plans_public_tag": "一般利用可 · 宿泊者以外も歓迎",
      "plans_save": "+ 旅程",
      "plans_details": "詳細"
    }
  },
  ...
  function currentPageKey() {
    return document.documentElement.dataset.svPage || document.body?.dataset.svPage || 'index';
  }

  function dictionaryForLang(lang) {
    ...
    return { ...baseDictionary, ...pageDictionary };
  }

  function isGuestHubContextActive() {
    const pageKey = currentPageKey();
    if (pageKey === 'guesthub') {
      try { sessionStorage.setItem('sv_guesthub_context', 'guesthub'); } catch (_) {}
      document.body?.classList.add('is-guesthub-context');
      return true;
    }

    const hasContextClass = Boolean(
      document.body?.classList.contains('is-guesthub-context') ||
      document.documentElement.classList.contains('is-guesthub-context')
    );

    let storedContext = '';
    try {
      storedContext = sessionStorage.getItem('sv_guesthub_context') ||
        sessionStorage.getItem('sv_safety_return_context') ||
        sessionStorage.getItem('sv_guest_logged_in') || '';
    } catch (_) {}

    const active = hasContextClass || storedContext === 'guesthub' || storedContext === 'true' || storedContext === '1';
    if (active) document.body?.classList.add('is-guesthub-context');
    return active;
  }
  ...
  function syncGuestHubContextLinks(lang) {
    const activeLang = normalizeLang(lang);
    const dictionary = dictionaryForLang(activeLang);
    const isGuest = isGuestHubContextActive();

    document.querySelectorAll('[data-guesthub-link], .sv-guesthub-link').forEach((link) => {
      const href = isGuest ? (link.dataset.guestHref || 'guesthub.html') : (link.dataset.defaultHref || 'hotel.html');
      const labelKey = isGuest ? 'nav_guesthub' : 'nav_guesthub_signin';
      link.setAttribute('href', href);
      const labelTarget = link.querySelector('span');
      if (labelTarget) {
        link.removeAttribute('data-i18n');
        labelTarget.setAttribute('data-i18n', labelKey);
      } else {
        link.setAttribute('data-i18n', labelKey);
      }
      link.classList.toggle('guesthub-active-context', isGuest);
      setLinkLabel(link, dictionary[labelKey] || (isGuest ? 'Guest Hub' : 'Guest Hub / Sign In'));
    });

    document.querySelectorAll('[data-context-return]').forEach((link) => {
      const href = isGuest ? (link.dataset.guestHref || 'guesthub.html') : (link.dataset.defaultHref || 'hotel.html');
      const labelKey = isGuest ? 'context_return_guest' : 'context_return_hotel';
      link.setAttribute('href', href);
      link.textContent = dictionary[labelKey] || (isGuest ? '← Back to Guest Hub' : '← Back to Hotel Website');
    });
  }
  ...
  function applyLanguage(lang) {
    ...
    syncGuestHubContextLinks(activeLang);
